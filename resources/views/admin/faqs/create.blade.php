...
@section('title', __('admin.add_faq_title'))
@section('content')
<div class="container-fluid">
    <div class="card">
        <div class="card-header"><h3 class="card-title">{{ __('admin.add_faq_title') }}</h3></div>
        <div class="card-body">
                            ...
                            @foreach ($langs as $key => $lang)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="custom-tabs-{{ $key }}" role="tabpanel">
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea name="question_{{ $key }}" class="form-control @error('question_'.$key) is-invalid @enderror" rows="3" {{ $key=='ar' || $key=='en' ? 'required' : '' }}>{{ old('question_' . $key) }}</textarea>
                                        @error('question_'.$key) <span class="invalid-feedback">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="form-group mb-0">
                                        <label>{{ __('admin.answer') }} ({{ $lang }}) @if($key=='ar' || $key=='en') <span class="text-danger">*</span> @endif</label>
                                        <textarea name="answer_{{ $key }}" class="form-control @error('answer_'.$key) is-invalid @enderror" rows="4" {{ $key=='ar' || $key=='en' ? 'required' : '' }}>{{ old('answer_' . $key) }}</textarea>
                                        @error('answer_'.$key) <span class="invalid-feedback">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                            @endforeach
                        </div>
