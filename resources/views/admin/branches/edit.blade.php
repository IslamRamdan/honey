...
@section('title', __('admin.edit_branch'))
@section('content')
<div class="container-fluid">
    <div class="card">
        <div class="card-header"><h3 class="card-title">{{ __('admin.edit_branch') }}</h3></div>
        <div class="card-body">
            <form action="{{ route('admin.branches.update', $branch) }}" method="POST">
                            ...
                            @foreach ($langs as $key => $lang)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="custom-tabs-{{ $key }}" role="tabpanel">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" name="country_{{ $key }}" class="form-control" value="{{ old('country_' . $key, $branch->{'country_' . $key}) }}" {{ $key=='ar' || $key=='en' ? 'required' : '' }}>
                                    </div>
                                    <div class="form-group mb-0">
                                        <label>{{ __('admin.description') }} ({{ $lang }}) @if($key=='ar' || $key=='en') <span class="text-danger">*</span> @endif</label>
                                        <textarea name="description_{{ $key }}" class="form-control" rows="4" {{ $key=='ar' || $key=='en' ? 'required' : '' }}>{{ old('description_' . $key, $branch->{'description_' . $key}) }}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="country_code" class="form-control" value="{{ old('country_code', $branch->country_code) }}">
                    </div>
                    <div class="form-group col-md-4">
                    ...
                    <div class="form-group col-md-4 d-flex align-items-end">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" {{ $branch->is_active ? 'checked' : '' }}>
